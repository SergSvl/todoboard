...
const useDraggable = () => {
  ...
  const onDrop = (event, dropElement, allElements, boardId = undefined) => {
    ...
    const dropElementType = dropElement.hasOwnProperty('cards') ? 'group' : 'card';
    ...
    if (dragElementId === dropElement.id 
      || (dragElementType === 'card' && dropElementType === 'group' && parentBoardId === dropElement.id)
      || (dragElementType === 'card' && dropElementType !== 'group' && parentBoardId !== boardId)) {
      return;
    }

    if (dragElementType === 'card' && dropElementType === 'group' && parentBoardId !== dropElement.id) {
      console.log("Карточка с одной доски перенесена на другую");
      console.log("DROP Element:", dropElement);
      ...
      const filteredBoards = deleteCardFromBoard(boards, parentBoardId, dragElementId);
